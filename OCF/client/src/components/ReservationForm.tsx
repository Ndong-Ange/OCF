...
const ReservationForm: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.guests < 1) newErrors.guests = 'Au moins 1 personne requise';

    // Vérifier que la date n'est pas dans le passé (jour uniquement)
    const selectedDateOnly = new Date(formData.reservation_date);
    const todayDate = new Date();
    todayDate.setHours(0, 0, 0, 0);
    selectedDateOnly.setHours(0, 0, 0, 0);
    if (selectedDateOnly < todayDate) {
      newErrors.reservation_date = 'La date ne peut pas être passée';
    }
    // (bloc supprimé) const selectedDate = new Date(formData.reservation_date + 'T' + formData.reservation_time');
    // (bloc supprimé) const now = new Date();
    // (bloc supprimé) if (selectedDate <= now) {
    // (bloc supprimé)   newErrors.reservation_date = 'La date doit être dans le futur';
    // (bloc supprimé) }
  ...
  return (
    <div className="bg-transparent p-6 rounded-lg shadow-md text-white">
